...
<body>
  ...
  $query = $conn->query("select temperature,time,deviceID from temperature where deviceID=1 limit 15");
  ...
  $query = $conn->query("select temperature,time,deviceID from temperature where deviceID=2 limit 15");
  foreach ($query as $data) {
    $temperature2[] = $data['temperature'];
  }

    $query = $conn->query("select round(avg(temperature),2) from (select temperature from temperature limit 15) as t;");
    foreach ($query as $data) {
      $temperature_avg = $data['round(avg(temperature),2)'];
    }

    $query = $conn->query("select round(max(temperature),2) from (select temperature from temperature limit 15) as t;");
    foreach ($query as $data) {
      $temperature_max = $data['round(max(temperature),2)'];
    }

    $query = $conn->query("select round(min(temperature),2) from (select temperature from temperature limit 15) as t;");
    foreach ($query as $data) {
